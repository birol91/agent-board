...
import type { HookEvent } from "@agentdeck/shared-types";
import { logError, logInfo } from "../log.js";
import { readProject } from "../fs/readProject.js";
import { resolveAgentNameFromTranscript } from "./resolveAgent.js";

const SOCKET_PATH = path.join(tmpdir(), "agentdeck.sock");
...
export async function startHookServer(): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    server!.listen(SOCKET_PATH, () => {
      void logInfo("hooks", `listening on ${SOCKET_PATH}`);
      resolve();
    });
  });
}
